...
func buildWebhookRetrievalHandler(db *sql.DB, client storage.Storer) http.HandlerFunc {
	// WebhookRetrievalHandler is a request handler that returns a single Webhook
...
func buildWebhookUpdateHandler(db *sql.DB, client storage.Storer) http.HandlerFunc {
	...
	return func(res http.ResponseWriter, req *http.Request) {
		...
		err = mergo.Merge(updatedWebhook, existingWebhook)
		if err != nil {
