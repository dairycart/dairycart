...
func (m *MockDB) GetProductVariantBridgeList(db storage.Querier, qf *models.QueryFilter) ([]models.ProductVariantBridge, error) {
	args := m.Called(db, qf)
	return args.Get(0).([]models.ProductVariantBridge), args.Error(1)
}

func (m *MockDB) GetProductVariantBridgeCount(db storage.Querier, qf *models.QueryFilter) (uint64, error) {
	args := m.Called(db, qf)
...
func (m *MockDB) ArchiveProductVariantBridgesWithProductRootID(db storage.Querier, id uint64) (t time.Time, err error) {
	args := m.Called(db, id)
	return args.Get(0).(time.Time), args.Error(1)
}
